...
export async function GET(req: Request) {
  ...
  const configs = bots.map(bot => {
    ...
    return {
      ...
      bot_id: bot.id,
      bot_name: bot.name,
      ssd_code_mtn: (found?.ssd_code_mtn && found.ssd_code_mtn.trim()) ? found.ssd_code_mtn : defaultMtnCode,
      ssd_code_moov: (found?.ssd_code_moov && found.ssd_code_moov.trim()) ? found.ssd_code_moov : defaultMoovCode,
